/** Enlaces públicos del proyecto. */
export const RUTA_UNIVERSITARIA = 'https://luma.com/unlock-ruta';
export const CALENDARIO_LUMA = 'https://luma.com/calendar/cal-YXLF6uFR0qMZ4Wi';
export const CALENDARIO_LUMA_EMBED =
  'https://luma.com/embed/calendar/cal-YXLF6uFR0qMZ4Wi/events';
export const SITIO_ZACATLAN = 'https://zacatlan.gob.mx/';
export const TELEGRAM_STEPH = 'https://t.me/STEPHSERRANO';
export const CORREO_ESCUELA = 'escuela.bitcoin.mexico@gmail.com';
/** Registro de wallet Bull Bitcoin con el referral de Steph Serrano. */
export const WALLET_BULLBITCOIN =
  'https://accounts.bullbitcoin.com/es/registration?referral_code=stephsatoshi';
export const SITIO_BULLBITCOIN = 'https://bullbitcoin.com';
/** Sitio oficial de UNLOCK Summit 2026. */
export const UNLOCK_SUMMIT_2026 = 'https://unlocksummit.io/unlock-summit-2026/';
